...
return interaction?.reply({ content: "Rate Limited.", ephemeral: true })
    ...
    function cmd_loader() {
        if (interaction?.type === InteractionType.ApplicationCommand) {
            ...
            if (!interaction?.member?.voice?.channelId) return interaction?.reply({ content: `Join a Voice channel`, ephemeral: true }).catch(e => { })
            ...
            return interaction?.reply({ content: `be in same VC`, ephemeral: true }).catch(e => { })
            }
            }
            }
