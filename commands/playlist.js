...
module.exports = {
  ...
  options: [
    {
      ...
      options: [
        ...
        {
          name: "public",
          description: "Want to make it Public? True/false",
    ...
    {
      name: "delete",
      description: "Album current album",
      type: ApplicationCommandOptionType.Subcommand,
      options: [
        {
          name: "name",
          description: "name of your Album to delete.",
          type: ApplicationCommandOptionType.String,
          required: true
        }
      ]
    ...
    {
      name: "add-music",
      description: "add songs to the Album.",
      type: ApplicationCommandOptionType.Subcommand,
      options: [
        {
          name: "playlist-name",
    ...
    {
      name: "delete-music",
      description: "Delete song in album",
      type: ApplicationCommandOptionType.Subcommand,
      options: [
        {
          name: "playlist-name",
        ...
        if (playlist?.length > 0) {
          for (let i = 0; i < playlist.length; i++) {
            if (playlist[i]?.playlist?.filter(p => p.name === name)?.length > 0) {
              return interaction.reply({ content: '⚠️ Album already Exist!', ephemeral: true }).catch(e => { })
            }
          }
        }
  ...
  .setDescription(`creation successful`)
  .setTimestamp();
...
await interaction.editReply({
  ...
  console.error('Error editing reply:', e);
      ...
      if (stp === "add-music") {
        let name = interaction.options.getString('name')
        if (!name) return interaction.reply({ content: 'Search song name', ephemeral: true }).catch(e => { })
        let playlist_name = interaction.options.getString('playlist-name')
        if (!playlist_name) return interaction.reply({ content: 'Enter album name to add songs', ephemeral: true }).catch(e => { })

        const playlist = await db.playlist.findOne({ userID: interaction.user.id }).catch(e => { })
        if (!playlist?.playlist?.filter(p => p.name === playlist_name).length > 0) return interaction.reply({ content: 'added', ephemeral: true }).catch(e => { })
       ...
       .setAuthor({
        ...
        .setDescription(`Addition successful`)
        await interaction.reply({
  content: '',
  embeds: [ loadingembed ] 
}).catch(e => {
...
await interaction.editReply({
  ...
  console.error('Error editing reply:', e);
      ...
      if (stp === "list") {
        let name = interaction.options.getString('name')
        if (!name) return interaction.reply({ content: 'Search album', ephemeral: true }).catch(e => { })
        ...
        for (let i = 0; i < playlist.length; i++) {
          if (playlist[i]?.playlist?.filter(p => p.name === name)?.length > 0) {
            ...
            if (!trackl?.length > 0) return interaction.reply({ content: 'Empty', ephemeral: true }).catch(e => { })
        ...
        const generateEmbed = async (start) => {
          ...
          if (!current || !current?.length > 0) return interaction.reply({ content: 'Empty', ephemeral: true }).catch(e => { })
      ...
      if (stp === "lists") {
        const playlist = await db?.playlist?.findOne({ userID: interaction.user.id }).catch(e => { })
        if (!playlist?.playlist?.length > 0) return interaction.reply({ content: `Error`, ephemeral: true }).catch(e => { })
      ...
      if (stp === "top") {
        ...
        if (!playlists?.length > 0) return interaction.reply({ content: 'No playlists', ephemeral: true }).catch(e => { })
        ...
        if (!trackl?.length > 0) return interaction.reply({ content: 'No playlists', ephemeral: true }).catch(e => { })
        ...
        const generateEmbed = async (start) => {
          let sayı = page === 1 ? 1 : page * kaçtane - kaçtane + 1
          const current = trackl.slice(start, start + kaçtane)
          if (!current || !current?.length > 0) return interaction.reply({ content: `no Albums`, ephemeral: true }).catch(e => { })
          return new EmbedBuilder()
            .setAuthor({
              name: 'Top Albums',
            })
        ...
        await interaction.reply({
          ...
          collector.on("collect", async (button) => {
            if (button.customId === "close") {
              collector.stop()
              return button.reply({ content: `stopped`, ephemeral: true }).catch(e => { })
            } else {
      ...
      console.error(e);
      interaction.reply({ content: 'Error occurred while the execution!', ephemeral: true }).catch(e => { })
    }
  }
}
